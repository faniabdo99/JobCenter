<?php
return [
  //APPLICATION NOT
  'NewJobApplicationInJobCenter' => 'تقديم جديد في الموقع',
  'YouHaveNewJobApplication' => 'لديك تقديم جديد على احدا الوظائف',
  'ApplicationDetails' => 'تفاصيل التقديم',
  'JobTitle' => 'العنوان الوظيفي',
  'Candidate' => 'المرشح للوظيفة',
  'ApplicationMessage' => 'رسالة التقديم',
  'GetTheFullApplicationDetailsFromYourApplicationsPage' => 'احصل على تفاصيل التقديم من خلال صفحة التقديم الخاصة بك!',
  'ViewProfile' => 'الملف التعريفي',
  'ApplicationsList' => 'قائمة التقديم',
  'Thanks' => 'شكراً',
  //contactus
  'NewMailFromJobCenter' => 'لديك ايميل جديد من موقع الJob Center',
  'From' => 'من',
  //contact
   'NewMailFromJobCenter' => 'لديك ايميل جديد من موقع الJobe Center',
   'From' => 'من',
   //quickcontact
   'NewEmail' => 'ايميل جديد',
   'YouRecivedThisEmailFromWomenJobCenterHomePage.' => 'لقد تلقيت هذا البريد الإلكتروني من الصفحة الرئيسية لمركز توظيف  النساء.',
   'MessageTitle:' => 'عنوان الرسالة :',
   'UserEmail:' => 'البريد الالكتروني للمستخدم :',
   'UserName:' => 'اسم المستخدم :',
   'Thanks' => 'شكراً',
 //reset Password
 'ResetPassword' => 'اعادة تعيين كلمة المرور',
 'Hello' => 'مرحباً ,',
 'YouHaveRequstedAPasswordResetToYourJobCenterAccount' => 'لقد طلبت إعادة تعيين كلمة المرور لحساب مركز التوظيف الخاص بك ، يرجى النقر فوق الزر أدناه لإعادة تعيين كلمة المرور الخاصة بك.',
 'ResetPassword' => 'اعادة تعيين كلمة المرور',
 'IfYouDidntMadeThisRequsteDontWorryJustIgnoreThisEmailAndYourPasswordWontBeChanged' => 'إذا لم تقدم هذا الطلب فلا تقلق ، فما عليك سوى تجاهل هذا البريد الإلكتروني لن يتم تغيير كلمة المرور الخاصة بك.',
 'Thanks' => 'شكراً',
 //user blade
 'Welcome' => 'مرحباً',
 'HelloThere' => 'مرحباً بك',
 'WelcomeToJobCenterPortal' => 'اهلا وسهلا بك في لوحة تحكم الخاصة بمركز التوظيف.',
 'YouHaveCreatedNewAccountWithThisEmailAndWeNeedYouToVerfiyItPleaseClickTheLinkBelowToVerifyYourAccount' => 'لقد قمت بإنشاء حساب جديد مع هذا البريد الإلكتروني ، ونريد التحقق منه ، يرجى النقر فوق الرابط أدناه للتحقق من حسابك.',
 'WelcomeToTheSite' => 'مرحباً بك في موقعنا!',
 'ActivateAccount' => 'تفعيل الحساب',
 'Thanks' => 'شكراً,',

];
